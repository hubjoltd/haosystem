# Hao System - ERP Application

## Overview
The Hao System is an enterprise resource planning (ERP) web application built with Angular 21 frontend and Spring Boot 3.2 backend. It provides comprehensive business management capabilities including inventory, customer, contract, HR, payroll, and administrative settings management.

## User Preferences
Preferred communication style: Simple, everyday language.

## System Architecture

### Frontend
- **Framework**: Angular 21 with TypeScript 5.9
- **Port**: 5000 (dev server with `ng serve --host 0.0.0.0 --port 5000`)
- **Proxy**: `/api` requests proxied to backend at `http://127.0.0.1:8080`
- **Config**: `frontend/angular.json` with `allowedHosts: true` for Replit proxy compatibility
- **Styling**: SCSS with dark navy/blue theme on login, teal (#008080) theme on dashboard

### Backend
- **Framework**: Spring Boot 3.2.0 with Java 17+ (runs on Java 19)
- **Port**: 8080 (localhost only)
- **Database**: PostgreSQL (Replit built-in, connected via PGHOST/PGPORT/PGUSER/PGPASSWORD env vars)
- **ORM**: Hibernate 6.3.1 with JPA
- **Auth**: JWT-based authentication with Spring Security
- **Build**: Maven (`mvn clean package -DskipTests`)

### Key Files
- `start.sh` - Startup script that launches backend JAR then Angular dev server
- `build.sh` - Build script that builds frontend, copies to backend static resources, then builds backend JAR
- `frontend/proxy.conf.json` - Proxy config routing `/api` to backend
- `backend/src/main/resources/application.properties` - Backend configuration
- `backend/src/main/java/com/erp/config/DataSeeder.java` - Sample data seeder (wrapped in try-catch for resilience)

### Deployment
- **Target**: Autoscale
- **Build**: Frontend built and copied to backend static resources, then backend packaged as JAR
- **Run**: `java -jar backend/target/erp-backend-1.0.0.jar --server.port=5000`
- In production, the Spring Boot JAR serves both the API and the static Angular frontend on port 5000

## Recent Changes
- **Employee login feature**: Employees can now log in using their Employee Code as username and a password set in the employee form. When a password is set, a User account with STAFF role is automatically created/updated. STAFF role users see only the Employee Service Portal (ESS) section.
- **Login page redesign**: HAO logo with dark blue/navy gradient, service category cards (Electrical, Access Control, CCTV, Networking, Servers, Communications), "Welcome Back" heading, "Email Address" label, "Sign in to Dashboard" button, removed company logos and "Remember Me" checkbox
- **Employee list columns reordered**: Employee Code, Name, Designation, Department, Project, Email, Status, Actions
- **Employee code auto-generation**: Uses branch code as prefix (or "EMP" default), generates sequential 4-digit codes via native SQL query. Fixed numeric extraction to handle dash-separated codes (EMP-001) and improved query sorting (LENGTH DESC, code DESC) for correct sequence.
- **Employee delete fix**: Cascade deletes related records (bank details, salary, education, experience, assets) before deleting employee
- **Employee list sorting**: Changed from DESC to ASC order
- **Onboarding assets**: Added asset endpoints to OnboardingController (GET /assets/employee/{id}, POST /assets, POST /assets/{id}/return)
- **Leave date fix**: Timezone-safe date parsing using component parts instead of new Date(string). Added WRITE_DATES_AS_TIMESTAMPS=false to serialize LocalDate as ISO strings instead of arrays.
- **HR Dashboard moved**: Moved from MIS & Dashboards section to HR Management as the first option in sidebar navigation
- **Real-time chat**: Added full backend support for employee chat with WebSocket (Spring WebSocket) for real-time messaging, ChatMessage entity stored in PostgreSQL, ChatController REST API for message history/send/read, JWT-authenticated WebSocket connections, typing indicators, and online status broadcasting
- **completeTask endpoint**: Made request body optional (@RequestBody(required = false))
- Wrapped DataSeeder.run() in try-catch to prevent Hibernate compatibility issues from crashing the application
- Created start.sh for development workflow (backend on 8080 + frontend dev server on 5000)
- Created .gitignore for Java and Node.js artifacts
- Configured deployment for autoscale with build step
- **Employee code fix**: Added dash separator in auto-generation (EMP-0004 format), added `/api/employees/next-code` endpoint for frontend to get the correct next available code
- **Leave date display fix**: formatDate() now handles both array `[2026, 2, 11]` and string `"2026-02-11"` formats with invalid date fallback
- **Company creation form**: Moved Company Name/Code to the top of the form (before logo and admin sections)
- **Chat security**: WebSocket now requires JWT token authentication only (removed userId query param fallback), with username-based user lookup for backward compatibility with old tokens
- **Employee code robust fallback**: 3-tier fallback: server `/api/employees/next-code` → prefix settings → employee list scan → default EMP-0001
- **Chat real-time via HTTP polling**: Replaced WebSocket with HTTP polling (3s interval) for reliable real-time chat. Polls `/api/chat/unread` for badge counts and `/api/chat/messages/{userId}` for conversation updates. Messages sent via REST POST with server-generated IDs.
- **Employee password save fix**: Fixed `NonUniqueResultException` caused by duplicate role names in database. Updated `RoleRepository.findByName()` and `findByNameAndBranch_Id()` to use `LIMIT 1` queries. Added duplicate-prevention checks in `RoleService.createDefaultRolesForBranch()`.
- **Geofence edit fix**: Added `locationType` field to AttendanceRecord model separate from `captureMethod`. When editing clock-in/out times, captureMethod changes to "EDITED" but locationType preserves the original location (ON_SITE, OFF_SITE). Edit modal now includes a Location dropdown. Frontend `isOnSite()` checks locationType first with captureMethod fallback for backward compatibility.
- **Employee login fix**: STAFF role users can now log in without a branch assigned. Login supports both employee code and email as username. Login form label updated to "Username or Email".
- **ESS timesheet flickering fix**: Clock timer interval (1s) now only triggers change detection when clock tab is active, preventing flickering on other tabs like timesheet.
- **Project update fix after adding members**: Fixed circular JSON reference (Project↔ProjectMember/Task/Milestone) using `@JsonManagedReference`/`@JsonBackReference`. Expanded `updateProject` PUT handler to support all fields (dates, billing, customer, project manager, location, permissions, etc.).
- **Comprehensive HR notifications**: Added notifications across all HR modules - Project (create, status change, member add, task assign), Leave (request, approve, reject), Payroll (processed - both admins and employees), Recruitment (requisition, candidate status, interview), Onboarding (plan create/complete), Training (session create, enrollment), Loan (submit, approve, reject), Expense (submit, approve, reject), HR Letter (generate), Attendance (approve). All wrapped in try-catch. Added `notifyEmployee()`, `notifyAdminsAndHR()` helpers to UserNotificationService. Added icons for PROJECT, PAYROLL, RECRUITMENT, ONBOARDING, TRAINING, HR_LETTER, EMPLOYEE notification types in header.
- **Employee project serialization fix**: Added `@JsonIgnoreProperties({"members", "tasks", "milestones", "customer", "projectManager"})` on Employee.project to prevent circular JSON and reduce payload size.
- **Calendar & Event Management**: Full-stack calendar with CalendarEvent entity (title, description, eventDate, startTime, endTime, eventType, color, location, meetingLink, attendees), CalendarEventRepository with date-range queries, CalendarEventController with CRUD endpoints at `/api/calendar/events`. Frontend CalendarComponent with Month/Week/Day views, animated transitions, mini calendar sidebar, upcoming events panel, time breakdown chart, click-to-create events, 6 event types (Meeting, Project, Event, Announcement, Training, Review) with color coding. Accessible via `/app/calendar` route with sidebar navigation link.
- **Calendar month view fix**: Fixed month grid display with overflow-y auto, increased month-week min-height to 100px, improved month-cell flexbox layout with flex-direction column and min-width 0.
- **Payroll project column**: Added "Project" column to payroll run details table showing employee's assigned project name with fallback chain.
- **HR Dashboard redesign**: Modern animated design with animated number counters (count-up from 0), auto-refresh every 30 seconds, glass-morphism card effects, multi-color gradient stat cards (purple/green/pink/blue/warm), SVG progress rings for attrition rate, animated bar charts with hover tooltips, "Recent Activity" timeline feed, shimmer skeleton loading states, and staggered CSS entry animations (fadeInUp, slideInLeft, scaleIn, pulse, shimmer).
- **Main Dashboard redesign**: Feature quick-access grid with 12 feature cards (HR, Projects, Calendar, Attendance, Payroll, Inventory, Contracts, Reports, Settings, Procurement, Accounting, Leave), each with multi-color gradient icon backgrounds, animated floating icons, click-to-navigate via router, time-based greeting section ("Good Morning/Afternoon/Evening"), improved stat cards with gradient icons, staggered fadeInUp animations, and glass-morphism styling. Responsive grid layout.
- **Employee project column fix**: Projects assigned via project_members junction table now show in Employee list, Attendance, Timesheet, and Payroll views. EmployeeController populates employee.project from ProjectMemberRepository when the direct project_id FK is null. AttendanceController similarly enriches projectName/projectCode on all GET endpoints and during clock-in/manual entry creation.
