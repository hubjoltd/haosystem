...
- **Geofence edit fix**: Added `locationType` field to AttendanceRecord model separate from `captureMethod`. When editing clock-in/out times, captureMethod changes to "EDITED" but locationType preserves the original location (ON_SITE, OFF_SITE, REMOTE, FIELD). Edit modal now includes a Location dropdown. Frontend `isOnSite()` checks locationType first with captureMethod fallback for backward compatibility.
